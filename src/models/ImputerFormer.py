...
def run_imputeformer(
    ...
    def predict_in_chunks(model_obj, x_input: np.ndarray, split_name: str) -> np.ndarray:
        chunk_size = int(config.get('imp_predict_chunk_size', 2000))
        if x_input.shape[0] <= chunk_size:
            return model_obj.predict({'X': x_input})['imputation']
        print(f'ImputeFormer: predição em chunks para {split_name} ({x_input.shape[0]} amostras).')
        preds = []
    ...
    d_model = int(config.get('imp_d_model', 128))
    n_heads = int(config.get('imp_n_heads', 4))
    if d_model % n_heads != 0:
        raise ValueError(f'ImputeFormer: d_model={d_model} precisa ser divisível por n_heads={n_heads}.')

    optimizer_name = str(config.get('imp_optimizer', 'adamw')).lower()
    lr = float(config.get('imp_lr', 1e-3))
    weight_decay = float(config.get('imp_weight_decay', 0.0))
    ...
    model = ImputeFormer(
        ...
        n_layers=int(config.get('imp_n_layers', 2)),
        d_input_embed=d_model,
        d_learnable_embed=d_model,
        d_proj=d_model,
        d_ffn=int(config.get('imp_d_ffn', 256)),
        n_temporal_heads=n_heads,
        dropout=float(config.get('imp_dropout', 0.1)),
        batch_size=int(config.get('imp_batch_size', 64)),
        epochs=int(config.get('imp_epochs', config.get('epochs', 50))),
        patience=int(config.get('imp_patience', config.get('patience', 10))),
        optimizer=optimizer,
        num_workers=int(config.get('num_workers', 0)),
        device=config.get('device', 'cpu'),
        saving_path=str(checkpoint_dir) if checkpoint_dir else None,
        model_saving_strategy=config.get('imp_model_saving_strategy', 'best') if checkpoint_dir else None,
    )

    checkpoint_loaded = False
    if checkpoint_dir and bool(config.get('resume_training_from_checkpoint', True)):
        checkpoint_loaded = try_load_checkpoint(model, Path(checkpoint_dir), model_name='ImputeFormer')
